...
export default function App() {
  ...
  const isAdmin      = isSuperAdmin; // alias cho export/import gate
      ...
      <BottomSheet isOpen={!!viewingMember} onClose={() => setViewingMember(null)} height="95vh">
        {viewingMember && (
          <MemberBottomSheet member={viewingMember} members={members}
            onClose={() => setViewingMember(null)} onEdit={handleEdit} isAdmin={canEdit} onSelectMember={setViewingMember} />
        )}
      </BottomSheet>

      <BottomSheet isOpen={canEdit && isFormOpen} onClose={() => { setIsFormOpen(false); setEditingMember(null); }} height="95vh">
        {canEdit && (
          <MemberForm isOpen={isFormOpen}
            onClose={() => { setIsFormOpen(false); setEditingMember(null); }}
            onSave={handleSave} onDelete={handleDelete}
            members={members} editingMember={editingMember} isAdmin={canEdit} isSuperAdmin={isSuperAdmin} />
        )}
      </BottomSheet>
